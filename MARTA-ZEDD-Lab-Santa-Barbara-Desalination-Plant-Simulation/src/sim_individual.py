...
class SBsim(object):
   ############# define relevant class parameters
    def __init__(self, opt_par, case_number, drought_type):
        ...
        self.demand      = annual_dem #monthly_demand(annual_dem)
        ...
        self.cost_curve_data = self.cost_curve_loader.load_cost_curve(self.case_number)
    ...
    def simulate(self, P, s):
        ...
        desal_capacity = desal_capacity = self.cost_curve_loader.get_max_production(self.case_number) #gross_capacity #- montecito_agreement
...
############## simulation of surface water reservoirs
            ...
            if deficit[t+1] < 1e-4:
                deficit[t+1] = 0
            if deficit[t+1] > 0: # hard constraint on deficit
                def_penalty += 10000
            ...
            Jrisk = - np.percentile(jrisk, 25) # risk objective formulated as mean of the storage. Lower mean storage equals higher risk of supply deficit. 
            Jcost = np.sum(desal_cost) + def_penalty
